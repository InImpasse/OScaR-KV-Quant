#!/usr/bin/env bash
# Run oscar-kv-bench from the project virtual environment without activation.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: ./scripts/bench.sh [wrapper options] [oscar-kv-bench args]
...
Wrapper options:
  ...
  -h, --help               Show this help and oscar-kv-bench --help if installed
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help) usage; BENCH_ARGS+=("$1"); shift ;;
    *) BENCH_ARGS+=("$1"); shift ;;
  esac
done
...
BENCH="${VENV_DIR}/bin/oscar-kv-bench"
