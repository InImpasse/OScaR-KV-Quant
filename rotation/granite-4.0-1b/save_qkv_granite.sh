#!/usr/bin/env bash
# Dump Q/K/V for Granite 4.0 1B — paths relative to OSCAR-KV-Quant repo root.
set -euo pipefail
...
MODEL="${MODEL:-${REPO_ROOT}/checkpoints/granite-4.0-1b-base}"
TP_SIZE="${TP_SIZE:-1}"
PORT="${PORT:-31110}"
...
MAX_QUEUED_REQUESTS="${MAX_QUEUED_REQUESTS:-16}"
MAX_WAIT_SECS="${MAX_WAIT_SECS:-1800}"

export DUMP_KVCACHE="${DUMP_KVCACHE:-true}"
export DUMP_KVCACHE_TOKENS="${DUMP_KVCACHE_TOKENS:-8000}"
...
# Rename calib dir (same logic as upstream qwen3-8B script)
